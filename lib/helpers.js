...
exports.sendKeys = function (req, res, resultArr, collectionType, baseType) {
	if (resultArr) {
		var keyArr = [];
		resultArr.forEach(function (result) {
			keyArr.push(result.key);
		})
		sendResponse(req, res, keyArr, collectionType, baseType);
	}
}

var sendResponse = exports.sendResponse = function (req, res, obj, collectionType, baseType) {
	switch (req.accepts(['json', 'text/xml', 'application/xml'])) {
		...
		case 'application/xml':
			...
			if (Object.prototype.toString.call(obj) === '[object Array]') {
				...
				obj = {}
				obj[baseType] = tempObj;
			}
			res.send(toXml(collectionType, obj));
			break;
		default:
