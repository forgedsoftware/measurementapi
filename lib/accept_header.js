module.exports = function (routes, staticPaths) {
	return function (req, res, next) {
		var errorFound, acceptHeader = [];
		
		// Split and attempt to process accept header
		req.headers.accept.split(' ').forEach(function (acceptType) {
			var match,
				headerRegex = /^(.+)\/((vnd\.(\w+))?(\.(.*))?\+)?(\w+)$/g; 
		...
		modifyUrl(req, res, routes, acceptHeader, staticPaths);
...
function modifyUrl (req, res, routes, acceptHeader, staticPaths) {
	if (acceptHeader.length > 0 && acceptHeader[0].version && req.url.indexOf('/v') !== 0) {
